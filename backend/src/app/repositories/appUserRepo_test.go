...
func TestAppUserRepo_Delete(t *testing.T) {
	type fields struct {
		database       db
		ID             string
		Del            bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := repo.Delete(tt.args.id, tt.args.opeUserID); (err != nil) != tt.wantErr {
				t.Errorf("AppUserRepo.Delete() error = %v, wantErr %v", err, tt.wantErr)
			}
			user, err1 := repo.SelectByID("appUserId1")
			if err1.Error() != "your search yielded no data" {
				t.Errorf("AppUserRepo.Delete() error = %v, wantErr %v", err1, tt.wantErr)
			}
